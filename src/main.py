...
from algorithm.natural.evolution import main_es as _main_es
...
def main():
    # Pre-parse --algo so we can delegate before constructing the full parser.
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--algo", type=str, default="ga_de",
                     choices=["ga_de", "es"])
    known, remaining = pre.parse_known_args()

    # ── Delegate to ES runner (strips --algo from sys.argv first) ───
    if known.algo == "es":
        import sys
    ...
    # ── Full GA/DE argument parser ───────────────────────────────────
    parser = argparse.ArgumentParser(
        description="Benchmark runner — GA/DE or ES algorithms"
    )
    parser.add_argument(
        "--algo", type=str, default="ga_de",
        choices=["ga_de", "es"],
        help="Algorithm family to run: 'ga_de' (default) or 'es'",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Benchmark problem — GA/DE only: sphere, rastrigin, ackley (default: ackley). Use --algo es for cigar/ridge.",
    )
    ...
    # ── Convergence history ──────────────────────────────────────────
    # GA/DE history stores best-solution vectors; re-evaluate to get fitness
