"""Compare optimization algorithms on the **Sphere** function (2-D).
...
-----
    cd src
    python -m comparision.compare_sphere [--cycle 200] [--runs 10] [--seed 42] [--save]
    python -m comparision.compare_sphere --save --output-dir outputs/run_01
    python -m comparision.compare_sphere --save --save-json
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Algorithm comparison on Sphere (2-D)")
    ...
    n_dim = 2
    problem = Sphere(n_dim=n_dim)
    print(f"Problem : Sphere  |  Dimensions : {n_dim}")
    ...
    if args.tune:
        ...
        tuned_params = load_tuned_config("Sphere")
        if tuned_params:
            print(f"\n>>> Loaded tuned config for Sphere ({len(tuned_params)} algos)")
        else:
            print(
    ...
    save_path = (
        make_output_path("compare_sphere.png", args.output_dir) if args.save else None
    )
