"""Compare optimization algorithms on the **Rastrigin** function (2-D).
...
-----
    cd src
    python -m comparision.compare_rastrigin [--cycle 200] [--runs 10] [--seed 42] [--save]
    python -m comparision.compare_rastrigin --save --output-dir outputs/run_01
    python -m comparision.compare_rastrigin --save --save-json
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Algorithm comparison on Rastrigin (2-D)"
    )
    ...
    n_dim = 2
    problem = Rastrigin(n_dim=n_dim)
    print(f"Problem : Rastrigin  |  Dimensions : {n_dim}")
    ...
    if args.tune:
        ...
        tuned_params = load_tuned_config("Rastrigin")
        if tuned_params:
            print(
                f"\n>>> Loaded tuned config for Rastrigin ({len(tuned_params)} algos)"
            )
        else:
            print(
    ...
    save_path = (
        make_output_path("compare_rastrigin.png", args.output_dir)
        if args.save
        else None
    )

    plot_comparison(results, problem_name="Rastrigin", n_dim=n_dim, save_path=save_path)

    if args.save_json:
        saved_json = save_results_json(
            results,
            make_data_output_path("compare_rastrigin.json", args.output_dir),
            metadata={
                "problem": "Rastrigin",
